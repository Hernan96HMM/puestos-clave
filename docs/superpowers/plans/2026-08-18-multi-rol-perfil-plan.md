...
5. `npm run dev`, then in a browser, using the credentials from `docs/superpowers/plans/2026-08-04-credenciales-prueba.md`:
   ...
   - If testing with an account that was already logged in before this deploy, log out and back in first — `esDireccion`/`sectoresGerente` are only set on fresh sign-in (same caveat Fase 3c had for `accesoExtendido`).
